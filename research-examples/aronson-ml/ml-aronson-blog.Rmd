...
```{r}
gbm_data <- vars_filt %>% 
  select(c(target, atrRatSlow, trend, apc10, deltaPVR3))
...
PnL_df$Date <- as.Date(as.character(rownames(gbm_data[(init+1):testSlices[[length(testSlices)]][horiz], ])), format="%d.%m.%Y")

# add underlying PnL curve
PnL_df$Underlying <- cumsum(gbm_data[(init+1):testSlices[[length(testSlices)]][horiz], "target" ])

# pivot data to long form
mlt <- melt(PnL_df, id="Date", value="PnL", variable_name="Threshold")
PnL_df_long <- PnL_df %>% 
  pivot_longer()
...
PnL_df_long %>% 
  ggplot(aes(x=Date, y=value)) +
    geom_line(aes(colou = Threshold, group = Threshold)) +
    labs(y = "PnL excl. Costs")
